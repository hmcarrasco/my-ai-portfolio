...
class TestOpenaiClient:
    ...
    def mock_prompts(self):
        ...
        with patch("ai.clients.openai_client.load_yaml", return_value={
    ...
    @pytest.mark.parametrize("model,temperature,max_messages", [
        ("gpt-4", 0.5, 10),
        ("gpt-3.5-turbo", 0.0, 5),
        ("gpt-4.1-mini", 0.7, 15),
    ])
    def test_initialization_with_parameters(self, mock_openai, mock_prompts, model, temperature, max_messages):
    ...
    def test_generate_response_with_memory_first_message(self, openai_client, mock_openai):
